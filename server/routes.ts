...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/apartments", async (req, res) => {
    try {
      ...
      const { minPrice, maxPrice, bedrooms, maxDistance, petFriendly } = req.query;
      ...
      if (minPrice) {
        filteredApartments = filteredApartments.filter(apt => apt.price >= Number(minPrice));
      }
      
      if (maxPrice) {
        filteredApartments = filteredApartments.filter(apt => apt.price <= Number(maxPrice));
      }
      
      if (bedrooms) {
        filteredApartments = filteredApartments.filter(apt => apt.bedrooms >= Number(bedrooms));
      }
  ...
  app.get("/api/search-parties", async (req, res) => {
    try {
      ...
      const searchParties = await storage.getSearchPartiesByUserId(Number(userId));
  ...
  app.post("/api/search-parties", async (req, res) => {
    try {
      ...
      if (!name || !createdById) {
        return res.status(400).json({ message: "Name and createdById are required" });
      }
  ...
  app.post("/api/search-parties/:id/members", async (req, res) => {
    try {
      ...
      if (!userId || !searchPartyId) {
        return res.status(400).json({ message: "UserId and searchPartyId are required" });
      }
  ...
  app.post("/api/search-parties/:id/listings", async (req, res) => {
    try {
      ...
      if (!apartmentId || !addedById || !searchPartyId) {
        return res.status(400).json({ message: "ApartmentId, addedById and searchPartyId are required" });
      }
